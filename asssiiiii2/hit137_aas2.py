"""
Assignment 2 (Question number 2)– Temperature Data Analysis

This program analyses temperature data collected from multiple weather
stations in Australia. Each CSV file in the "temperatures" folder represents
one year of data. The missing values (NaN) are ignored.

Functions:
1. Seasonal Average:
   - Calculates average temperature for each season (Summer, Autumn,
     Winter, Spring) across all stations and years.
   - Results written to 'average_temp.txt'.

2. Temperature Range:
   - Finds the station(s) with the largest range (max – min) temperature.
   - Results written to 'largest_temp_range_station.txt'.

3. Temperature Stability:
   - Identifies the most stable (lowest standard deviation) and most variable
     (highest standard deviation) stations.
   - Results written to 'temperature_stability_stations.txt'.

Usage:
    python temperature_analysis.py --input temperatures --output-dir results

Helpful flags:
-extract-zip   : Also search .zip files for a 'temperatures' folder.
-demo          : Force using a small, auto-generated demo dataset.
-strict        : If set and input folder is missing, exit with code 2.
                      (By default, the program now falls back to demo data.)
- run-tests     : Run internal self-tests and exit.

Dependencies:
    pandas, math, csv

Author: Shah Mohammad Sakin
Date: [within 4th of september]"""
...
# --
# Utilities for locating and loading data
# --
...
def find_temperatures_folder(preferred: str, allow_extract_zip: bool = False) -> Tuple[Optional[str], Optional[str]]:
    """Locate the temperatures folder; optionally extract it from a zip file.
 Returns a tuple of (folder_path, extracted_temp_dir). The second element is a
 temp directory to clean up if extraction/demo was used, otherwise None."""
...
def load_all_csvs(folder_path: str) -> pd.DataFrame:
    """Load and combine all CSV files from the given folder into one DataFrame.

    Expected columns: a station identifier column and monthly columns January..December.
    Non-numeric values are coerced to NaN and ignored by downstream calculations.
    """
    try:
        files = [
...
# ---
# Demo dataset helper (for default/--demo fallback)
# ---
...
def _run_internal_tests() -> None:
    ...
    # Test 1: Basic aggregation across two years and multiple stations
    ...
    # Test 2: Folder discovery by walking up the directory tree
    ...
    # Test 4: month_to_season mapping correctness
    ...
    # Test 5: Skipping a bad CSV without station/month columns shouldn't crash
    with tempfile.TemporaryDirectory() as td:
    ...
    # Test 6: Demo dataset helper produces a valid folder consumable by loader
    with tempfile.TemporaryDirectory() as td:
    ...
    # Test 7: A folder without CSVs should not be detected as valid
    with tempfile.TemporaryDirectory() as td:
    ...
    # Test 8: Station with only one numeric observation should have NaN stddev
    with tempfile.TemporaryDirectory() as td:
    ...
    # Test 9: Output files are written and non-empty for a tiny dataset
    with tempfile.TemporaryDirectory() as td:
...
def main():
    ...
    # If demo is explicitly requested, build it first.
    demo_tmp = None
    if args.demo:
        ...
        # Try to locate the folder using the requested strategy
        ...
        # Default behavior: fall back to demo unless --strict is set
    ...
    # Cleanup any extracted/demo temp directory
